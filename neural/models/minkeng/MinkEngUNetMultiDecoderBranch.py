...
class MinkEngUNetMultiDecoderBranch(torch.nn.Module):
    def __init__(
        self,
        channel_in,
        first_conv_channel,
        ...
        super(MinkEngUNetMultiDecoderBranch, self).__init__()
    ...
    def make_predict_module(self, decoder_output_channels, out_channels=1):
        module_list = []
    ...
    def forward(self, x):
        ...
        for i in range(stages):
            upsample = {}
            if i == 0:
                for value in self.upsample:  # x,y,z,p
                    upsample[value] = self.upsample[value][i](encoder_layers[-1])
            else:
                for value in self.upsample:
                    upsample[value] = self.upsample[value][i](
                        decoder_layers[value][i - 1]
                    )
            ...
            if i < (self.stages - 1):
                for value in self.upsample:
                    cat_upsample[value] = ME.cat(
                        (upsample[value], encoder_layers[-i - 2])
                    )
                    # print("\tcat upsample: ", upsample.F.shape, encoder_layers[-i-2].F.shape, cat_upsample.F.shape)
            else:
                for value in self.upsample:
                    cat_upsample[value] = ME.cat((upsample[value], inlet_layer))

            for value in decoder_layers:
                decoder_layers[value].extend(
                    [self.decoder[value][i](cat_upsample[value])]
                )

        # header
        val_out = {}
        for value in decoder_layers:
            val_out[value] = self.header[value](decoder_layers[value][-1])

        return ME.cat(val_out["X"], val_out["Y"], val_out["Z"], val_out["P"])
