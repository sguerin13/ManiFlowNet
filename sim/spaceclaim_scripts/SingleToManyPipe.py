...
save_dir = "Path/To/Geometries/SingleToMany/"
...
class Single_To_Many():
    ...
    def create_outlet_points(self):
        ...
        self.num_out = R.randint(1,self.pd['n_outlets_var']) + \
                  self.pd['n_outlets_min']

        self.dist_bet_out = (R.random()*self.pd['dist_bet_out_var'] + \
                        self.pd['dist_bet_out_min'])*self.out_circle_d
        ...
        # TODO: Add random noise to the locations?
        # add the rest of the outlet locations
        for i in range(1,self.num_out):
            self.outlet_list.Add(Point.Create(
                                 M(self.outlet_list[i-1].X + self.dist_bet_out),
                                 M(self.outlet_list[i-1].Y),
                                 M(self.outlet_list[i-1].Z))
                                )
        ...
        for i in range(self.num_out):
            ...
            if self.pipe_mode == 'interpolate':
                int_pnt = Point.Create(self.anchor_points[i].X + 
                          (self.outlet_list[i].X - self.anchor_points[i].X)*.85,
                           self.outlet_list[i].Y/3.0,
                           self.outlet_list[i].Z/3.0)

            if self.pipe_mode == 'z_lock':
                int_pnt = Point.Create(self.anchor_points[i].X + 
                          (self.outlet_list[i].X - self.anchor_points[i].X)*.6,
                           self.outlet_list[i].Y/4.0,
                           0)
            
            if self.pipe_mode == 'y_lock':
                int_pnt = Point.Create(self.anchor_points[i].X + 
                          (self.outlet_list[i].X - self.anchor_points[i].X)*.6,
                           0,
                           self.outlet_list[i].Z/4.0)
    ...
    def create_curves(self):
        ...
        for i in range(self.num_out):
            ...
            branch_start_tangent = Vector.Create(self.inlet_dir.X*self.in_circle_d*20.0,
                                                 self.inlet_dir.Y*self.in_circle_d*20.0,
                                                 self.inlet_dir.Z*self.in_circle_d*20.0)
            
            branch_end_tangent = Vector.Create(branch_dir.X*self.out_circle_d*20.0,
                                               branch_dir.Y*self.out_circle_d*20.0,
                                               branch_dir.Z*self.out_circle_d*20.0)
    ...
    def save_file(self):
        ...
        '''
        save_file_str = '1_to_' + str(self.num_out) + '_' + \
...
'dist_bet_out_min'  : 2.0,
'dist_bet_out_var'  : 1.0,
...
i = 0
pipe_mode_list = ['interpolate','z_lock','y_lock']
while i < n_geo_to_build:
        
    pipe = Single_To_Many(pipe_dict)
    pipe.pipe_mode = R.choice(pipe_mode_list)
    pipe.build_pipe()
